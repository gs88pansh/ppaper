...
class GRU4Rec:
    ...
    def __init__(self, layers, n_epochs=10, batch_size=50, dropout_p_hidden=0.5, learning_rate=0.01, momentum=0.0,
    ...
    def init(self, data):
        ...
        for i in range(len(self.layers)):
            ...
            self.Bh.append(
                theano.shared(value=np.zeros((self.layers[i] * 3,), dtype=theano.config.floatX), borrow=True))
    ...
    def model(self, X, H, Y=None, drop_p_hidden=0.0):
        ...
        for i in range(1, len(self.layers)):
            ...
            h = self.hidden_activation(
                T.dot(H[i] * rz[:self.layers[i]].T, self.Wh[i]) + vec.T[:self.layers[i]].T)  # CHK
    ...
    def fit(self, data, retrain=False, sample_store=10000000):
        ...
        if not retrain:
            ...
            if n_new_items:
                ...
                self.By.set_value(
                    np.vstack([self.By.get_value(), np.zeros((n_new_items, 1), dtype=theano.config.floatX)]))
