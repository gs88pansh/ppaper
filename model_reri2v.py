...
class Re_RI2V(object):
    ...
    def rnn_layer(self,X,name="rnn_layer",time_major=False,):
        ...
        """
        cell = tf.nn.rnn_cell.GRUCell(num_units=self.rnn_states)
    ...
    def rnn_out_layer(self,X,final_X,name="rnn_out_layer"):
        with tf.name_scope(name):
            ...
            X = tf.reshape(X,[-1,self.rnn_states])
            rnn_output_W = tf.Variable(tf.random_uniform([self.rnn_states, self.embedding_size], -1, 1), name='rnn_output_W')
